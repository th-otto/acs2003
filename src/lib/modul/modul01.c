...
/*                      (simplified version for new style modules only)       */
/*                                                                            */
...
int16 Ash_module(const char *path)
{
	...
	if ((mod->magic1 >= 0x41435330L && mod->magic1 <= 0x41435338l) || /* 'ACS0', 'ACS8' */
		/* this is clr.w -(a7); trap #1 */
		(mod->magic1 != 0x42674E41L || mod->magic2 != 0x41435339L) || /* ACS9 */
		PushFuncLists(mod) == FALSE)
	{
		Mfree(pd);
		if (mygemdos)
			oldgemdos();
	...
	if (mygemdos)
		oldgemdos();
	/* just go */
	if (Pexec(4, NULL, (void *)pd, NULL) != 0)
	{
		if (mygemdos)
			accgemdos();
		Mfree(pd);
